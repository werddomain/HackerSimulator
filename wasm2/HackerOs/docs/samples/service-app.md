...
to session cancellation signals, publishes typed events to `IAppEventGateway`, and cleans up without leaving
volatile state across session restarts.
...
| `SampleTickerEvent.cs` | Typed event published to `IAppEventGateway` on every tick |
...
SampleTickerService.RunAsync
      ...
      │      ├── Publish SampleTickerEvent(TickCount, Timestamp, Status)
