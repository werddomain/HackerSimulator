 ...
 * @param container - The container element for the system monitor
 * @param switchTabCallback - Callback to switch tabs 
 */
export function initMobilePerformanceMonitoring(
  container: HTMLElement, 
  switchTabCallback?: (tab: string) => void
): MobileMonitoringResult {
   ...
   */
  function updateRenderingMetrics(): void {
   ...
   */
  function drawChart(canvas: HTMLCanvasElement, data: number[], color: string): void {
   ...
   */
  function getColorForFrameTime(frameTime: number): string {
    if (frameTime <= 16.67) {
   ...
   */
  function getColorForJank(jankScore: number): string {
    if (jankScore <= 2) {
   ...
   */
  function getJankClass(jankScore: number): string {
    if (jankScore <= 2) {
      return 'metric-good';
    } else if (jankScore <= 5) {
      return 'metric-warning';
