...
import { initDOMPerformanceOptimization, getDOMPerformanceObserver } from './dom-performance-observer.js';
import * as CSSOptimizer from './mobile-css-optimizer.js';
...
export class MobilePerformanceOptimizer {
  ...
  detectDeviceCapabilities() {
    ...
    if (navigator.gpu && navigator.gpu.wgslLanguageFeatures) {
   ...
   * @param {PerformanceMetrics} metrics - Updated performance metrics
   */
  onFrameCallback(metrics) {
   ...
   */
  applyHardwareAccelerationToElement(element) {
   ...
   */
  setupAccelerationCleanup(element) {
    if (!element) return;
  ...
  reduceAnimationComplexity() {
    ...
    animatedElements.forEach(element => {
      ...
      const hasExpensiveAnimations = DOMOptimizer.LAYOUT_THRASHING_PROPERTIES.some(
        prop => style.getPropertyValue(`transition-property`).includes(prop) ||
  ...
  optimizeListsAndTables() {
    ...
    lists.forEach(list => {
      ...
      if (itemCount > 50) {
        ...
        if (observer && observer.virtualizedLists && !observer.virtualizedLists.has(list)) {
          // Force the DOM observer to virtualize this list
          observer.optimizeList(list);
        }
      }
    });
  }
   ...
   */
  applyAdditionalHardwareAcceleration() {
    ...
    if (element.children.length > 0) {
      ...
      Array.from(element.children).forEach(child => {
        ...
        if (needsOptimization) {
          DOMOptimizer.optimizeForAnimation(child);
        }
      });
    }
  }
 ...
 * @returns {MobilePerformanceOptimizer} - Optimizer instance
 */
let optimizerInstance = null;
export function initMobilePerformanceOptimizer(config = {}) {
 ...
 * @returns {MobilePerformanceOptimizer|null} - The current optimizer instance or null
 */
